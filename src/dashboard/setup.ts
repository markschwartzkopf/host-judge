...
(document.getElementById('reset-hyperdeck') as HTMLButtonElement).onclick =
  () => {
    nodecg.sendMessage('resetHyperdeck').catch((err) => {
      nodecg.log.error(err);
    });
  };
...
hyperdeckAddressInput.onkeyup = (e) => {
  switch (e.key) {
    case 'Enter':
      ...
      break;
    case 'Escape':
      NodeCG.waitForReplicants(hyperdeckIp)
        .then(() => {
...
hyperdeckClips.on('change', () => {
  drawSegments();
});

function drawSegments() {
  NodeCG.waitForReplicants(auditionSegments, hyperdeckClips)
    .then(() => {
